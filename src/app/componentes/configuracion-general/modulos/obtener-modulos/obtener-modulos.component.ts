...
@Component({
  ...
  templateUrl: './obtener-modulos.component.html',
  styleUrl: './obtener-modulos.component.css',
  providers: [MessageService],
})
export class ObtenerModulosComponent {
  ...
  displayedColumns: string[] = ['opciones', 'id', 'modulo', 'sala', 'piso', 'creado'];
  ...
  nuevo(){
    ...
    this.router.navigateByUrl('/infoclic/configuracion-general/crear-modulo')
  }
  ...
  editar(objeto: any){
    ...
    this.router.navigate(['/infoclic/configuracion-general/crear-modulo'], {
      state: { datos: objeto }
    });
  }
  ...
  peticion(url:string){   
    ...
    this.peticionsServicios.peticionPOST(url, datos).subscribe({  
            ...
            if(data.Data.length == 0){
              ...
              if(url == '/obtener-modulos'){
                ...
                if (!Array.isArray(this.modulos)) {
                  this.modulos = Object.values(this.modulos);
                }

                const index = this.modulos.findIndex(s => s.id == id);
                if (index !== -1) {
                  this.modulos[index].activo = activo === 1 ? 0 : 1;
      ...
      error: (err) => {
        this.mensaje = "Error inesperado al obtener módulos.";
        this.mensajeErrorServicios.actualizarError(this.mensaje, '');
