...
@Component({
  ...
  styleUrl: './obtener-clientes.component.css',
  providers: [MessageService],
})
export class ObtenerClientesComponent {
  ...
  displayedColumns: string[] = ['opciones', 'id', 'documento', 'nombre', 'creado'];
  ...
  nuevo(){
    ...
    this.router.navigateByUrl('/infoclic/configuracion-general/crear-cliente')
  }
  ...
  editar(objeto: any){
    ...
    this.router.navigate(['/infoclic/configuracion-general/crear-cliente'], {
      state: { datos: objeto }
    });
  }
  ...
  peticion(url:string){   
    ...
    this.peticionsServicios.peticionPOST(url, datos).subscribe({  
        ...
        if(data.Status == 200){
          if(data.Error == true){
            ...
            if(data.Data.length == 0){
              ...
              if(url == '/obtener-clientes'){
                this.clientes = data.Data.map((s:any) => ({
                  ...s,
                ...
                if (!Array.isArray(this.clientes)) {
                  this.clientes = Object.values(this.clientes);
                }

                const index = this.clientes.findIndex(s => s.id == id);
                if (index !== -1) {
                  this.clientes[index].activo = activo === 1 ? 0 : 1;
      ...
      error: (err) => {
        this.mensaje = "Error inesperado al obtener clientes.";
        this.mensajeErrorServicios.actualizarError(this.mensaje, '');
