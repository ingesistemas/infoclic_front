...
@Component({
  ...
  templateUrl: './obtener-centros.component.html',
  styleUrl: './obtener-centros.component.css',
  providers: [MessageService],
})
export class ObtenerCentrosComponent {
  ...
  displayedColumns: string[] = ['opciones', 'id', 'centro', 'creado'];
  ...
  nuevo(){
    ...
    this.router.navigateByUrl('/infoclic/configuracion-general/crear-centro')
  }
  ...
  editar(objeto: any){
    ...
    this.router.navigate(['/infoclic/configuracion-general/crear-centro'], {
      state: { datos: objeto }
    });
  }
  ...
  peticion(url:string){   
    ...
    this.peticionsServicios.peticionPOST(url, datos).subscribe({  
        ...
        if(data.Status == 200){
          if(data.Error == true){
            ...
            if(data.Data.length == 0){
              ...
              if(url == '/obtener-centros'){
                this.centros = data.Data.map((s:any) => ({
                  ...s,
                ...
                if (!Array.isArray(this.centros)) {
                  this.centros = Object.values(this.centros);
                }

                const index = this.centros.findIndex(s => s.id == id);
                if (index !== -1) {
                  this.centros[index].activo = activo === 1 ? 0 : 1;
      ...
      error: (err) => {
        this.mensaje = "Error inesperado al obtener centros.";
        this.mensajeErrorServicios.actualizarError(this.mensaje, '');
