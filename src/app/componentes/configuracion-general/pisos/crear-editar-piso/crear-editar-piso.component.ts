...
@Component({
  selector: 'app-crear-editar-piso',
  imports: [ReactiveFormsModule, ToastModule],
  templateUrl: './crear-editar-piso.component.html',
  styleUrl: './crear-editar-piso.component.css',
  providers: [MessageService],
})
export class CrearEditarPisoComponent {
 ...
 // mostrarCargandoSucursales: boolean = false
  ...
  formulario = this.fb.group({
    id: [0],
    piso: ['' , [Validators.required, Validators.minLength(5)]],
  ...
  ngOnInit(): void {
    ...
    if(this.tamanioForm.accionActual() == 'Editar'){
      ...
      this.formulario.controls['piso'].setValue(datos.datos.piso)
    }
  }
  ...
  aceptar(){
    if(this.accion == 'Crear'){
      this.peticion('/crear-piso')
    }else{
      this.peticion('/editar-piso')
    }
  }
